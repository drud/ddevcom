...
if ( ! class_exists('Schema_WP_SpecialPage_CheckoutPage') ) :
	/**
	 * Schema Checkout Page
	 *
	 * @since 1.0.0
	 */
	class Schema_WP_SpecialPage_CheckoutPage {
		...
		*/
		public function output_no_markup( $schema ) {
			...
			$page_id = schema_wp_get_option( 'checkout_page' );
		...
		*/
		public function output_markup() {
			...
			$enabled = apply_filters( 'schema_wp_output_CheckoutPage_enabled', true );
				if ( ! $enabled )
					return;
			...
			if ( is_singular() ) {
				...
				do_action( 'schema_wp_action_CheckoutPage' );
		...
		*/
		public function get_markup() {
			...
			return apply_filters( 'schema_singular_output_CheckoutPage', $schema );
		}
		...
		*/
		public function schema_output( $post_id = null ) {
			...
			$schema['alternativeHeadline']	= get_post_meta( $post->ID, 'schema_properties_CheckoutPage_alternativeHeadline', true );
			...
			return apply_filters( 'schema_output_CheckoutPage', $schema );
		}
	}
	
	new Schema_WP_SpecialPage_CheckoutPage();
