...
function schema_wp_admin_extensions_page() {
	...
	$add_ons_tabs = (array) apply_filters( 'schema_wp_extensions_tabs', array(
		'official-premium'	=> __('Premium', 'schema-premium'),
		//'official-x'		=> __('Official Free', 'schema-premium'), // remove this for now! (not needed)
	) );

	$active_tab = isset( $_GET['tab'] ) && array_key_exists( $_GET['tab'], $add_ons_tabs ) ? $_GET['tab'] : 'official-premium';

	ob_start();
	?>
	<div class="wrap" id="schema-wp-extensions">
		<h1>
			<?php _e( 'Extensions for', 'schema-premium' ); echo ' ' . SCHEMAPREMIUM_PLUGINNAME; ?>
			<span>
				&nbsp;&nbsp;<a href="https://schema.press/downloads/?utm_source=plugin-extensions-page&utm_medium=plugin&utm_campaign=Schema%20Extensions%20Page&utm_content=All%20Extensions" class="button-primary" title="<?php _e( 'Browse all extensions', 'schema-premium' ); ?>" target="_blank"><?php _e( 'Browse all extensions', 'schema-premium' ); ?></a>
			</span>
		</h1>
		...
		<div id="tab_container">
			...
			<?php if ( 'official-premium' === $active_tab ) : ?>
				<p><?php printf( __( '<b style="color:green">Coming Soon!</b> Premium Extensions are only available with a Professional or Ultimate license. If you already have one of these licenses, simply <a href="%s">log in to your account</a> to download any of these extensions.', 'schema-premium' ), 'https://schema.press/account/?utm_source=plugin-extensions-page&utm_medium=plugin&utm_campaign=Schema%20Extensions%20Page&utm_content=Account' ); ?></p>
				<p><?php printf( __( 'If you have a Personal or Plus license, you can easily upgrade from your account page to <a href="%s">get access to all of these extensions</a>!', 'schema-premium' ), 'https://schema.press/account/?utm_source=plugin-extensions-page&utm_medium=plugin&utm_campaign=Schema%20Extensions%20Page&utm_content=Account' ); ?></p>
			<?php else : ?>
				<p><?php _e( 'Our official free extensions are available to all license holders!', 'schema-premium' ); ?></p>
			<?php endif; ?>
 ...
 * @return void
 */
function schema_wp_extensions_get_rest( $tab = 'official-premium' ) {
		...
		if ( ! is_wp_error( $api_response ) ) {
			...
			if ( $extensions && ! empty($extensions['products'] ) ) {
				...
				foreach ( $extensions['products'] as $key => $extension ) {
					...
					$cache .= '<p>'.$excerpt.'</p>';
					$cache .= '<a href="'.$ext_url.'?utm_source=plugin-extensions-page&amp;utm_medium=plugin&amp;utm_campaign=SchemaExtentionsPage&amp;utm_content='.$info['title'].'" title="Recurring Payments" class="button-secondary">'.__('Get this Extension', 'schema-premium').'</a>';
					$cache .= '</div>';
				} //end foreach
			...
			$cache = '<div class="error"><p>' . __( 'There was an error retrieving the extensions list from the server. Please try again later.', 'schema-premium' ) . '</div>';
		}
	} //end if ( false === $cache )  
