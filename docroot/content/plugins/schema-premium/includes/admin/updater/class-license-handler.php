...
if ( ! class_exists( 'Schema_Premium_License' ) ) :

/**
 * EDD_License Class
 */
class Schema_Premium_License {
	private $file;
	private $license;
	private $item_name;
	private $item_id;
	...
	private function includes() {
		if ( ! class_exists( 'Schema_WP_SL_Plugin_Updater' ) )  {
			require_once 'class-extension-updater.php';
		}
		
		require_once 'class-enable-beta.php';
	}
	...
	private function hooks() {
		...
		add_filter( 'schema_beta_enabled_extensions', array( $this, 'register_beta_support' ) );
	...
	public function auto_updater() {
		...
		$betas = schema_wp_get_option( 'enabled_betas', array() );
		
		$args = array(
			'version'   => $this->version,
			'license'   => $this->license,
			'author'    => $this->author,
			'beta'      => function_exists( 'schema_premium_get_beta_enabled_extensions' ) && schema_premium_get_beta_enabled_extensions( $this->item_shortname ),
		);
	...
	public function settings( $settings ) {
		...
		$edd_license_settings = array(
			array(
				...
				'name'    => sprintf( __( '%1$s', 'schema-premium' ), $this->item_name ),
				'desc'    => '',
				'type'    => 'license_key',
	...
	public function license_help_text( $active_tab = '' ) {
		...
		echo '<p>' . sprintf(
			__( 'Enter your extension license keys here to receive updates for purchased extensions. If your license key has expired, please <a href="%s" target="_blank">renew your license</a>.', 'schema-premium' ),
			'https://schema.press/'
		) . '</p>';
	...
	public function deactivate_license() {
		...
		if( ! wp_verify_nonce( $_REQUEST[ $this->item_shortname . '_license_key-nonce'], $this->item_shortname . '_license_key-nonce' ) ) {
			...
			wp_die( __( 'Nonce verification failed', 'schema-premium' ), __( 'Error', 'schema-premium' ), array( 'response' => 403 ) );
	...
	public function notices() {
		...
		if( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
			...
			if( empty( $_GET['tab'] ) || 'licenses' !== $_GET['tab'] ) {
				...
				$messages[] = sprintf(
					__( 'You have invalid or expired license keys for Schema. Please go to the <a href="%s">Licenses page</a> to correct this issue.', 'schema-premium' ),
	...
	public function plugin_row_license_missing( $plugin_data, $version_info ) {
		...
		if( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_imissing_key_message[ $this->item_shortname ] ) ) {
			...
			echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'admin.php?page=schema&tab=licenses' ) ) . '">' . __( 'Enter valid license key for automatic updates.', 'schema-premium' ) . '</a></strong>';
